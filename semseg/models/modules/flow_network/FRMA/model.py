...
class FRMA(torch.nn.Module):
    ...
    def forward(self, F, Fw, f, F0_c, KD=None):
        B, C, T, H, W = F.shape
        F = self.rdb(F)
        # print("F shape 1: ", F.shape)       # B C=dim T=5 H=110 W=160
        # print("Fw shape 1: ", Fw.shape)     # B C=feature_dim H=110 W=160
        # print("F0_c shape: ", F0_c.shape)   # B C=feature_dim T=1 H=110 W=160
...
class flow_network(torch.nn.Module):
    ...
    def __init__(self, config, feature_dim):
        ...
        self.event_feature_extractor = nn.Sequential(nn.Conv3d(in_channels, dim, kernel_size=[1,3,3], padding=[0,1,1], stride=1, bias=bias),
                                               nn.LeakyReLU(negative_slope=0.2, inplace=True),
                                            #    RRDB(dim=dim, num_RDB=num_RDB, growth_rate=growth_rate, num_dense_layer=num_dense_layer, bias=config.bias)
                                               )
        self.rgb_feature_extractor = nn.Sequential(nn.Conv2d(3, dim, kernel_size=3, padding=1, stride=1, bias=bias),
                                                  nn.LeakyReLU(negative_slope=0.2, inplace=True),
                                                #   RRDB(dim=dim, num_RDB=num_RDB, growth_rate=growth_rate, num_dense_layer=num_dense_layer, bias=config.bias)
                                                  )

        # FRMA blocks for drgradation learning network
        self.FRMA_blocks = nn.ModuleList([FRMA(dim, feature_dim, num_seq, growth_rate, num_dense_layer, num_flow, num_transformer_block, num_heads,
    ...
    def forward(self, event_voxel, Fw, F0_c=None):
        ...
        # print(F0_c.shape)               # B C=feature_dim T=1 H=110 W=160
        f = None

        for i, blk in enumerate(self.FRMA_blocks):
            # print("Start FRMA block ", i)
            # print(event_feature.shape, Fw.shape, F0_c.shape)
            event_feature, Fw, f = blk(event_feature, Fw, f, F0_c)
        # Fw = rearrange(Fw, 'b (c t) h w -> b c t h w', t=T, c=self.dim // T)
        # KD = self.d_conv(Fw)

        f_Y = self.f_conv(f)
        # anchor = self.a_conv(event_feature)
